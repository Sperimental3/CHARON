...
ntu120_perm = np.array([17, 39, 42, 41, 48, 37, 6, 49, 51, 38, 22, 40, 29, 34, 32, 2, 54,
                        36, 1, 57, 13, 10, 16, 15, 12, 24, 30, 0, 23, 7, 43, 52, 28, 11,
                        8, 45, 58, 35, 59, 56, 18, 53, 3, 31, 55, 14, 20, 50, 27, 25, 33,
                        47, 26, 9, 4, 44, 46, 19, 21, 5, 74, 102, 90, 67, 75, 105, 66, 107, 71, 86, 80, 115, 95,
                        100, 94, 92, 118, 91, 116, 60, 114, 64, 109, 78, 69, 99,
                        89, 73, 108, 83, 76, 62, 104, 112, 101, 117, 84, 110, 63,
                        93, 111, 61, 81, 97, 119, 70, 79, 113, 103, 68, 106, 72,
                        96, 88, 98, 82, 65, 77, 85, 87])
...
class Ntu120(Dataset):
    def __init__(self, data_path, p_interval=1, split='train', window_size=-1, transform=None, permute=True):
        ...
        if permute:
            self.targets = ntu120_perm[self.targets]
    ...
    def load_data(self):
        ...
        for i in trange(self._data.shape[0], desc=f'Pre-processing {self.split} data for NTU120'):
...
class SequentialNtu120(ContinualDataset):
    NAME = 'seq-ntu120'
    SETTING = 'class-il'
    ...
    def __init__(self, args):
        ...
        self.train_dataset = Ntu120(self.args.data_path, split="train",
                                    p_interval=[0.5, 1], window_size=120,
                                    transform=self.TRANSFORM)

        self.test_dataset = Ntu120(self.args.data_path, split="test",
...
class JointNtu120(SequentialNtu120):
    NAME = 'joint-ntu120'
    N_CLASSES_PER_TASK = 120
    N_TASKS = 1
