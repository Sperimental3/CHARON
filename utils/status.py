...
class ProgressBar:
    ...
    def prog(self, i: int, max_iter: int, epoch: Union[int, str],
        ...
        if i:  # not (i + 1) % 10 or (i + 1) == max_iter:
            ...
            progress_scale = int(50 * progress)
            progress_bar = ('█' * progress_scale) + ('┈' * (50 - progress_scale))
            ep_per_hour = round(3600 / (self.running_sum / i * max_iter), 2)
            task_str = task_number + 1 if isinstance(task_number, int) else task_number
            print(
                f'\r[{datetime.now().strftime("%m-%d | %H:%M")}]',
                f'Task {task_str} | ep {epoch}: |{progress_bar}|',
                f'{ep_per_hour:>2.2f} ep/h |',
                f' {i + 1:>{len(str(max_iter))}}/{max_iter} it |',
                f'loss: {round(loss, 6)} |',
                file=sys.stderr,
                end='',
                flush=True)
